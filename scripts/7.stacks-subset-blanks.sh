...
## rerun with the blanks 

#### popfile needs updating ####
# leave the cat group without a blank
POPMAP1=$src/pop-subset.noblank.txt
# this has all the files
POPMAP2=$src/pop-final.txt

# I have moved the blank files into this dir
DATA=$src/data/adtrim.stdlen2
...
#######
# Error here because EDDA02-1A catches both EDDA02-1A.1.fq.gz and EDDA02-1A2.1.fq.gz so the ustacks
# line fails as two sequence files are add at the -f switch. I fixed this by running this manually for
# EDDA02-1A with the ID=71. You need to fix this in future runs.
# Fixed by removing wildcard as shown below
#######
...
do
    #### set m, M, N ####
	# removed wildcard from {sample}*.1 to avoid EDDA02 issues
    echo "running $sample with ustacks using id $id"
    ustacks -m 3 -M 3 -N 5 -t gzfastq -f $DATA/${sample}.1.fq.gz \
	    -o $src/stacks3 -i $id --name $sample -p 32
    let "id+=1"
done
...
#### set n ####
echo "Running cstacks on sub-population of high coverage samples (5 each)"
date

cstacks -n 3 -P $src/stacks3/ -M $POPMAP1 -p 100

#
# Run sstacks. Match all samples supplied in the population map against the catalog.
#
echo "Running sstacks on entire set of samples"
date
sstacks -P $src/stacks3/ -M $POPMAP2 -p 100
...
tsv2bam -P $src/stacks3/ -M $POPMAP2 --pe-reads-dir $DATA -t 100
...
gstacks -P $src/stacks3/ -M $POPMAP2 -t 32 
...
populations -P $src/stacks3/ -M $POPMAP2 --vcf \
